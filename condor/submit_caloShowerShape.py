...
Unlike submit_ntupler.py there is no --algo / --jz / --special-jz0: the shower
shape ntupler just reads EtaSK GEP objects + truth BSM, so the only per-sample
knobs are the signal label (used to tag the output) and signal vs background.
...
def main():
    ...
    # For background/dijet samples --label distinguishes JZ slices etc.
    if args.background:
        label = args.label if args.label else "background"
    else:
        label = args.label if args.label else args.signal
    # signal_string is the tag the macro stamps on its output ROOT file. For a
    # real signal it is the signal key; for background it is the (optional) label
    # (empty -> the wrapper/macro fall back to the "background" tag).
    signal_string = ("" if args.background and not args.label
                     else (label if args.background else args.signal))

    # Fill in any omitted paths from the standard CaloShowerShapeTriggers layout.
    # Only --signal samples have a known sample sub-directory; --background must be
    # pointed at its inputs explicitly.
    if not args.daod_dir or not args.gep_dir:
        if args.background:
            parser.error("--background requires explicit --daod-dir and --gep-dir")
